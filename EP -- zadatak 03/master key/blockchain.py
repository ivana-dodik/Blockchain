...
class Blockchain:
    def __init__(self, mnemonic_words=None):
        ...
        self.transactions = []
        self.transactions_count = 0 # Used to generate derivated keys
        ...
        self.nodes = set()
        if mnemonic_words:
            self.master_private_key = HDKey.from_passphrase(mnemonic_words)
            self.master_public_key = self.master_private_key.public()
            self.address = self.master_public_key.address()
    ...
    def add_transaction(self, sender, receiver, amount):
        ...
        transaction = {
            ...
            'receiver': receiver,
            'amount': amount,
            'timestamp': str(time.time()) # Used to make each transaction unique otherwise useless
        }

        txid = hashlib.sha256(str(transaction).encode()).hexdigest()

        self.transactions_count += 1
        child_private_key = self.master_private_key.child_private(index=self.transactions_count)
        child_public_key = child_private_key.public()

        # Sign the transaction
        signature = Signature.create(txid=txid, private=child_private_key)

        # Add the signature and public key to the transaction
        transaction['txid'] = txid
        transaction['signature'] = signature.hex()
        transaction['public_key'] = child_public_key.hex()
